    ...
    <div class="add-content" :class="{ 'admin-view': currentUserRole === 'superadmin' || currentUserRole === 'sysadmin' }">
        ...
        <el-tabs v-model="activeTab" @tab-change="handleTabChange">
              ...
              <div class="approval-item" v-if="form.approveRecords[0] && ['林业局通过', '林业局驳回'].includes(form.status)">
                <div class="approval-header" style="padding: 10px 15px; background: #f9f9f9; display: flex; align-items: center">
                  <span
                    :class="[
                      'status-icon',
                      form.approveRecords[0].lyjApproveResult === '通过' ? 'success' : form.approveRecords[0].lyjApproveResult ? 'error' : 'pending'
                    ]"
                  >
                    {{ form.approveRecords[0].lyjApproveResult === '通过' ? '✓' : form.approveRecords[0].lyjApproveResult ? '✗' : '—' }}
                  </span>
                ...
                <div class="approval-content">
                  ...
                  <div class="feedback-item">
                    <span class="label">反馈文件：</span>
                    <div class="file-list">
                      <template v-if="parseApprovalFile(form.approveRecords[0].lyjApprovalAttachment).length">
                        <el-link
                          v-for="file in parseApprovalFile(form.approveRecords[0].lyjApprovalAttachment)"
                          :key="file.ossId"
                          :href="file.url"
                          :underline="false"
                          target="_blank"
          ...
          <el-tab-pane label="二次审批信息" name="feedback" v-if="form.approveRecords && form.approveRecords.length > 1">
            ...
            <div class="approval-info">
              <h3 class="section-title">审批信息</h3>
              <!-- 二次审批记录列表（排除首次） -->
              <div v-for="(record, index) in form.approveRecords.slice(1)" :key="record.id || index" class="approval-item">
                <!-- 管委会审批 -->
      ...
      <div v-else>
        ...
        <div class="project-documents" v-if="showApprovalSection && !!form.approveRecords?.[0]?.gwhApproveResult">
          <h3 class="section-title">审批信息</h3>
          <!-- 循环渲染每一条审批记录 -->
          <div v-for="(record, index) in displayApprovalRecord" :key="record.id || `approval-record-${index}`" class="approval-record-item">
            ...
            >
              ...
              <el-form-item label="审批反馈">
                <el-input type="textarea" :value="record.lyjApprovalReason || '暂无反馈'" :rows="2" style="background: #fff" disabled />
              </el-form-item>
              <el-form-item label="反馈文件">
                <transition-group class="upload-file-list el-upload-list el-upload-list--text" name="el-fade-in-linear" tag="ul">
                  <li
                    v-for="(file, fileIndex) in parseFileList(record.lyjApprovalAttachment)"
                    :key="file.ossId || fileIndex"
                    class="el-upload-list__item ele-upload-list__item-content"
                  >
                    <el-link :href="file.url" :underline="false" target="_blank">
                  ...
                  <li
                    v-if="!record.lyjApprovalAttachment || parseFileList(record.lyjApprovalAttachment).length === 0"
                    class="el-upload-list__item"
                    key="'empty-lyjFeedback-' + index"
                  >
                    <span class="el-icon-info"> 暂无反馈文件 </span>
                  </li>
                </transition-group>
...
// 新增：计算属性 - 筛选需要显示的审批记录（单条：原记录/最后一条）
const displayApprovalRecord = computed(() => {
  ...
  if (approvalRecords.length === 0) {
    ...
    return [approvalRecords[0]]; // 1条记录时，返回包含该条记录的数组
  } else {
    return [approvalRecords[approvalRecords.length - 1]]; // 大于1条时，返回包含最后一条记录的数组
  }
});
// 获取文件名（处理路径）
