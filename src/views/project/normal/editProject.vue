...
onMounted(async () => {
  ...
  try {
    ...
    // 从字符串解析文件列表（假设后端返回的是文件ID数组的JSON字符串）
    ...
    if (projectData.publicMaterial) {
      try {
        publicMaterialFileList.value = JSON.parse(projectData.publicMaterial)
      } catch (e) {
...
const handleUploadSuccess = (res, file, type) => {
  if (res.code === 200) {
    console.log("🚀 ~ handleUploadSuccess ~ file.name:", file.name)
    ...
    console.log("🚀 ~ handleUploadSuccess ~ fileItem:", fileItem)
...
const handleDeleteUploadFile = async (index, type) => {
  ...
  switch (type) {
    case 'locationPlan':
      fileId = locationPlanFileList.value[index].id
      locationPlanFileList.value.splice(index, 1)
      break
    case 'expertOpinions':
      fileId = expertOpinionsFileList.value[index].id
      expertOpinionsFileList.value.splice(index, 1)
      break
    case 'publicMaterial':
      fileId = publicMaterialFileList.value[index].id
      publicMaterialFileList.value.splice(index, 1)
      break
    case 'siteSelectionReport':
      fileId = siteSelectionReportFileList.value[index].id
      siteSelectionReportFileList.value.splice(index, 1)
      break
    case 'approvalDocuments':
      fileId = approvalDocumentsFileList.value[index].id
      approvalDocumentsFileList.value.splice(index, 1)
      break
    case 'projectRedLine':
      fileId = projectRedLineFileList.value[index].id
      projectRedLineFileList.value.splice(index, 1)
      break
    case 'redLineCoordinate':
      fileId = redLineCoordinateFileList.value[index].id
      redLineCoordinateFileList.value.splice(index, 1)
      break
    case 'threeDModel':
      fileId = threeDModelFileList.value[index].id
...
const resetForm = async () => {
  try {
    ...
    publicMaterialFileList.value = projectData.publicMaterial ? JSON.parse(projectData.publicMaterial) : []
  ...
  infoFormRef.value?.validate(async (valid) => {
    if (valid) {
      ...
      try {
        ...
        proxy?.$modal.msgError("更新失败：" + (err.message || "未知错误"))
      } finally {
        buttonLoading.value = false
      }
    }
...
defineExpose({
  open: (row) => {
    if (row) {
      ...
      publicMaterialFileList.value = row.publicMaterial ? JSON.parse(row.publicMaterial) : []
